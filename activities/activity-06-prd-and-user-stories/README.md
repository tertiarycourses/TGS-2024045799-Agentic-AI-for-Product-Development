...
- Required output: Lean PRD, story map and acceptance set
...
- 5. Execute the activity-specific workflow: Restate problem → Define behavior → Write stories → Add examples → Set non-goals → Review gaps.
...
- Pass condition: Requirements are testable, non-goals are explicit, and every priority story has three acceptance examples.
