...
- Required output: Workflow canvas and responsibility matrix
...
- 5. Execute the activity-specific workflow: Define goal → Map stages → Assign tools → Set schemas → Add gates → Set stop.
...
- Pass condition: Every stage has an owner and observable output; consequential actions stop for human review.
