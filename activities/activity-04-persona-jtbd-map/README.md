...
- Required output: Two persona cards, JTBD statements and validation plan
...
- 5. Execute the activity-specific workflow: Cluster needs → Draft persona → Write JTBD → Label inference → Rank risk → Plan test.
...
- Pass condition: Persona needs trace to source IDs; assumptions are labelled; each risky assumption has a feasible validation test.
