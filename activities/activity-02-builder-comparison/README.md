...
- Required output: Weighted builder comparison with sensitivity check
...
- 5. Execute the activity-specific workflow: Freeze task → Set criteria → Score builders → Apply weights → Test sensitivity → Recommend.
...
- Pass condition: Scores use one common task, weights total 100%, and the recommendation survives or explains sensitivity.
